...
public class CartMain extends javax.swing.JPanel {
    ...
    private void cmdDeleteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdDeleteActionPerformed
        ...
        if (selectedRows.length == 0) {
            // Tidak ada baris yang dipilih, tampilkan pesan peringatan
            MessageAlerts.getInstance().showMessage("Warning", "No item selected", MessageAlerts.MessageType.ERROR);
            return;
        }

        // Menampilkan dialog konfirmasi
        MessageAlerts.getInstance().showMessage("Confirmation", "Delete this order?", MessageAlerts.MessageType.WARNING, MessageAlerts.YES_NO_OPTION, (pc, x) -> {
            if (x == MessageAlerts.YES_OPTION) {
                DefaultTableModel model = (DefaultTableModel) tableCart.getModel();
