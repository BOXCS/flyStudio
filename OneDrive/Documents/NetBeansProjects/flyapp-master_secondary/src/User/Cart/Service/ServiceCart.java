...
public class ServiceCart {
    ...
    public boolean updateTransactionStatus(String productName, String transactionSig) {
        ...
        try {
            ...
            System.err.println("SQLException occurred while updating transaction status for product: " + productName);
            ex.printStackTrace();
            return false;
        } catch (Exception ex) {
            // Tangani kesalahan lainnya
            System.err.println("Exception occurred while updating transaction status for product: " + productName);
            ex.printStackTrace();
            return false;
        }
    }
    ...
    public boolean checkoutProduct(String productName, String transactionSig) {
        // Query SQL untuk memperbarui status transaksi dalam tabel transaction
        String updateTransactionQuery = "UPDATE transaction SET status = 'Waiting', type = ? WHERE product_name = ? AND status = 'Carted'";
        // Query SQL untuk menghapus item dari tabel cart berdasarkan nama produk
        String deleteCartItemQuery = "DELETE FROM cart WHERE transaction_number = ?";
        ...
        try {
            ...
            try (PreparedStatement pstmtUpdateTransaction = connection.prepareStatement(updateTransactionQuery); PreparedStatement pstmtDeleteCartItem = connection.prepareStatement(deleteCartItemQuery)) {

                // Set parameter untuk query update transaksi
                pstmtUpdateTransaction.setString(1, transactionSig);
                pstmtUpdateTransaction.setString(2, productName);

                // Set parameter untuk query hapus item dari tabel cart
                pstmtDeleteCartItem.setString(1, productName);

                // Eksekusi perintah update transaksi
                int rowsAffectedTransaction = pstmtUpdateTransaction.executeUpdate();

                // Jika ada baris yang terpengaruh di tabel transaksi, lanjutkan
                if (rowsAffectedTransaction > 0) {
                    // Eksekusi perintah hapus item dari tabel cart
                    int rowsAffectedCartItem = pstmtDeleteCartItem.executeUpdate();

                    // Log hasil eksekusi
                    System.out.println("Rows affected in transaction: " + rowsAffectedTransaction);
                    System.out.println("Rows affected in cart: " + rowsAffectedCartItem);

                    // Jika ada baris yang terpengaruh di kedua tabel, commit transaksi
                    if (rowsAffectedCartItem > 0) {
                        connection.commit();
                        return true;
                    } else {
                        // Jika tidak, rollback transaksi
                    ...
                    // Jika tidak ada baris yang terpengaruh di tabel transaksi, rollback transaksi
                    connection.rollback();
                    return false;
                }
            } catch (SQLException ex) {
