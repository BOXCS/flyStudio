...
public class ServiceMyOrder {
    ...
    public static void fetchCancelledOrders(DefaultTableModel model, String loggedInUsername) {
        ...
        try {
            // Query untuk mendapatkan data dari tabel transaction dengan status Cancelled, Reported Cancel, Denied Cancel, dan Accepted Cancel
            String sql = "SELECT transaction_number, username, product_name, level, designer, created_at, amount, status FROM transaction WHERE (status = 'Cancelled' OR status = 'Reported Cancel' OR status = 'Denied Cancel' OR status = 'Accepted Cancel') AND username = ?";
            PreparedStatement p = DatabaseConnection.getInstance().getConnection().prepareStatement(sql);
            p.setString(1, loggedInUsername); // Set parameter username pengguna yang login
            ResultSet r = p.executeQuery();
...
//                parameters.put("status", status);
    ...
    public static String loadReason(String transactionNumber) {
        String reason = "";

        try {
            // Query untuk mendapatkan alasan pembatalan dari tabel canceltransaction berdasarkan transaction_number
            ...
            e.printStackTrace();
            reason = "Error retrieving reason from database.";
        }
