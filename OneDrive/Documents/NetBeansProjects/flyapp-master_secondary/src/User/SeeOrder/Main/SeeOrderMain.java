...
import User.SeeOrder.Component.CacellationView;
...
import User.SeeOrder.Swing.CellLate.CellLateEditor;
import User.SeeOrder.Swing.CellLate.CellLateRenderer;
import User.SeeOrder.Swing.CellLate.TableActionLateEvent;
        ...
        TableActionCancelledEvent eventC = new TableActionCancelledEvent() {
            ...
            public void onReason(int row) {
                ...
                if (selectedRow != -1) {
                    ...
                    // Mendapatkan Frame atau Dialog induk
                    Window parentWindow = SwingUtilities.getWindowAncestor(SeeOrderMain.this);
                    if (parentWindow instanceof Frame) {
                        Frame parentFrame = (Frame) parentWindow;
                        // Membuka CacellationView dengan transactionNumber
                        CacellationView dialog = new CacellationView(parentFrame, true, transactionNumber.toString());
                        dialog.setVisible(true);
                    } else {
                        JOptionPane.showMessageDialog(SeeOrderMain.this, "Parent window is not a Frame.");
                    }
                } else {
                    JOptionPane.showMessageDialog(SeeOrderMain.this, "Please select a row first.");
                }
...
//                    Object type = tableActive.getValueAt(selectedRow, 0);
        ...
        tableLate.getColumnModel().getColumn(8).setCellEditor(new CellLateEditor(eventL));
    }
