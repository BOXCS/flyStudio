...
import Admin.AddAdmin.Component.RFIDAddAdmin;
        ...
        cmdToAdmin.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if (selectedRows.length > 0) {
                    try {
                        // Tampilkan dialog RFIDAddAdmin dan dapatkan RFID tag
                        RFIDAddAdmin rfidDialog = new RFIDAddAdmin((Frame) SwingUtilities.getWindowAncestor(cmdToAdmin), true);
                        rfidDialog.setVisible(true);
                        String rfidTag = rfidDialog.getRFIDTag();

                        // Pastikan RFID tag tidak null
                        if (rfidTag != null && !rfidTag.isEmpty()) {
                            // Persiapkan query untuk menyisipkan data ke tabel admin
                            String insertQuery = "INSERT INTO admin (username, email, password, rfid_tag) VALUES (?, ?, ?, ?)";
                            ...
                            for (int i = 0; i < selectedRows.length; i++) {
                                ...
                                String password = (String) tableDesigner.getValueAt(selectedRows[i], 0); // Anggap kolom password ada di indeks 0
                                String username = (String) tableDesigner.getValueAt(selectedRows[i], 1); // Anggap kolom username ada di indeks 1
                                String email = (String) tableDesigner.getValueAt(selectedRows[i], 2); // Anggap kolom email ada di indeks 2

                                // Sisipkan nilai username, email, password, dan rfid_tag ke dalam tabel admin
                                insertStatement.setString(1, username);
                                insertStatement.setString(2, email);
                                insertStatement.setString(3, password);
                                insertStatement.setString(4, rfidTag);
                            ...
                            JOptionPane.showMessageDialog(null, "RFID not provided. Insert operation cancelled.", "Error", JOptionPane.ERROR_MESSAGE);
                        }
