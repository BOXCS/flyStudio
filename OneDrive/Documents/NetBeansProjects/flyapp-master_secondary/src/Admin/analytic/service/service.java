...
public class service {
    ...
    public List<DesignerAnalytics> getDesignerAnalytics() throws SQLException {
        Connection conn = getConnection();
        PreparedStatement stmt = null;
        ResultSet rs = null;
        List<DesignerAnalytics> analyticsList = new ArrayList<>();

        try {
            // Query the view to get designer analytics data
            String query = "SELECT username, level, month, monthly_income "
                    + "FROM view_designeractivities "
                    + "WHERE status = 'Finished' "
                    + "ORDER BY username, month DESC";
            stmt = conn.prepareStatement(query);
            rs = stmt.executeQuery();
            ...
            while (rs.next()) {
                ...
                String month = rs.getString("month");
                double totalAmount = rs.getDouble("monthly_income");

                // Convert month to "MMMM yyyy" format
                DateTimeFormatter dateFormatter = DateTimeFormatter.ofPattern("MMMM yyyy", Locale.ENGLISH);
                String formattedMonth = LocalDate.parse(month + "-01", DateTimeFormatter.ofPattern("yyyy-MM-dd")).format(dateFormatter);

                DesignerAnalytics analytics = new DesignerAnalytics(username, level, totalAmount, formattedMonth);
